...
function CobrancasPage() {
  ...
  const vencidasFiltradas = useMemo(() => {
    ...
    if (!termo) return vencidas as any[];

    return (vencidas as any[]).filter((m) => {
      const textoBusca = [
        m.alunos?.nome,
        m.alunos?.telefone,
        m.alunos?.email,
        m.valor,
        m.data_vencimento,
        m.data_vencimento ? formatDate(m.data_vencimento) : "",
        m.valor ? formatBRL(m.valor) : "",
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  const enviarIndividual = useMutation({
    mutationFn: async (m: any) => {
      ...
      await supabase.from("mensagens").insert({
        ...
        tipo: "cobranca",
        canal: "whatsapp",
        conteudo: `Cobrança ${formatBRL(m.valor)} venc. ${
          m.data_vencimento
        }`,
  ...
  const enviarLote = useMutation({
    mutationFn: async () => {
      ...
      if (list.length === 0) {
        throw new Error("Selecione ao menos uma cobrança");
      }
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Mensagens disparadas via webhook do n8n.
          </p>
        </div>

        <Button
          onClick={() => enviarLote.mutate()}
          disabled={selected.size === 0 || enviarLote.isPending}
        >
          <Users className="mr-2 size-4" />
          Enviar em lote ({selected.size})
        </Button>
      </header>
        ...
        <CardHeader className="space-y-4">
          ...
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Buscar por aluno, telefone, valor ou vencimento..."
              className="pl-10"
            />
          </div>
          ...
          {busca.trim() && (
            <p className="text-xs text-muted-foreground">
              Mostrando {vencidasFiltradas.length} resultado
              {vencidasFiltradas.length === 1 ? "" : "s"} para “{busca}”.
            </p>
          )}
        </CardHeader>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow>
                    ...
                    >
                      {busca.trim()
                        ? "Nenhuma cobrança encontrada para essa busca."
                        : "Nenhuma mensalidade vencida 🎉"}
                    </TableCell>
                  </TableRow>
                ) : (
                  vencidasFiltradas.map((m) => (
                    <TableRow key={m.id}>
                      <TableCell>
                        <Checkbox
                      ...
                      <TableCell className="text-right">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => enviarIndividual.mutate(m)}
                          disabled={enviarIndividual.isPending}
                        >
